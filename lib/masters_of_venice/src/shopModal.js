  ...
  render() {
    ...
    return (
      ...
      >
        <h5>{shop.name}</h5>
        <div className="stats">
          <span className="resource_price">
            Price:<br/>
            <TextField autoFocus={(this.props.shop_type === 'resource_price') ? true : false} className="shop_price" 
              inputStyle={ {width: 100, fontSize: 40, height: 60} } 
              style={{width: 100, fontSize: 40, height: 60}} 
              key={uuidV4()} 
              name="shop_price" type="number" value={shop.resource_price} 
              onChange={updateStat.bind(this, 'resource_price', {min: 5, max: 100, val: 5})} />
          </span>
