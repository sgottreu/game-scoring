    ...
    if(stat.quantity > 0){
      ...
      state.players.forEach( (player, p) => {
        if(player.stocks[ key ].quantity > 0){
          state.players[ p ].tmpDividends += player.stocks[ key ].quantity * dividend;
        }
      });
...
export var calculateVP = (state, cp) => {
  ...
  if(state.players[ cp ].guildOrders > 5){
    ...
    vp = state.players[ cp ].guildOrders + 2;
  }
  return vp;
}
...
export var guild_hall = (state) => {
  ...
  state.players[ cp ].tmpVP = calculateVP(state, cp);
...
export var shipping_offices = (state) => {
  ...
  state.players.forEach( (player, p) => {
    if(player.stocks[ office ].quantity > 0){
      ...
      state.players[ p ].tmpDividends += payment;
