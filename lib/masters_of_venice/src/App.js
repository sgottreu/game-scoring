...
class App extends Component {
  ...
  addPlayer = (e,i,v) => {
    ...
    if(state.currentName !== ''){
      state.players.push(
        { name: state.currentName, stocks: clone(dividends), bank: 150, vp: 0, tmpPayments: 0, tmpVP: 0, guildOrders: 0 }
      );
  ...
  changeActionEvent = (key, e) => {
    let state = this.state;
    state.actionStats[ key ].action = parseInt(e.target.value, 10);
  ...
  render() {
    let ss = this.state.selectedShop;

    let _i = this.state.shops.findIndex((s) => { return s.key === ss });
