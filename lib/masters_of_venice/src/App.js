...
const prepState = (state) => {
  ...
  delete _state.user;
  delete _state.availableGames;
  delete _state.game_oid;
...
class App extends Component {
  ...
  clearState = (state) => {
    ...
    let _dbState = prepState(clone(state));
  ...
  joinGame = () => {
    let state = this.state;
    state.currentName = state.user.name;

    state = this.addPlayer(false, state);
  ...
  updateGameState = (dbState) => {
    ...
    Object.entries(dbState).forEach(([key, value]) => {  
  ...
  selectGameInstance = (event, index) => {
    this.setState( { game_oid: (index === 0) ? 'new' : this.state.availableGames[index-1]._id} );
  };
